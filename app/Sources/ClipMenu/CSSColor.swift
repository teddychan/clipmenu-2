...
// Color-string resolver for JS actions' clip.set/addStringAttributes (color
// values). Faithful port of the legacy GTM category NSColor+String.m
// (Apache-2.0): a CSS/X11 named-color table, else #RRGGBB[AA] hex, else
// rgb(r,g,b[,a]). Used by ScriptableClip.

enum CSSColor {

    /// `NSColor colorWithString:` (NSColor+String.m:colorWithString 257-282).
    static func color(_ string: String) -> NSColor? {
        guard !string.isEmpty else { return nil }
        if let value = names[string.lowercased()] {
            return color(fromULong: UInt64(value), hasAlpha: false)
        }
        return colorFromHex(string) ?? colorFromRGB(string)
    }

    // value is 0xRRGGBB or 0xRRGGBBAA (ColorWithUnsignedLong, :194-208).
    private static func color(fromULong value: UInt64, hasAlpha: Bool) -> NSColor {
        var v = value
        var a: CGFloat = 1.0
        if hasAlpha {
    ...
    // ColorWithHexDigits (:211-227): skip to the first hex char, then take the
    // *contiguous* hex run (matching NSScanner scanUpTo/scanChars); len>=6 → RGB,
    // len==8 → +alpha. (A mere filter would mis-read "rgb(…)" or stray letters.)
    private static func colorFromHex(_ string: String) -> NSColor? {
        let lower = string.lowercased()
        let hexChars = Set("0123456789abcdef")
        guard let start = lower.firstIndex(where: hexChars.contains) else { return nil }
        var end = start
        while end < lower.endIndex, hexChars.contains(lower[end]) {
            end = lower.index(after: end)
        }
        let hex = String(lower[start..<end])
        guard hex.count >= 6, let value = UInt64(hex, radix: 16) else { return nil }
        return color(fromULong: value, hasAlpha: hex.count == 8)
    }

    // ColorWithCSSString (:230-254): rgb(r,g,b[,a]) — components 0-255, alpha 0-1.
    private static func colorFromRGB(_ string: String) -> NSColor? {
        let lower = string.trimmingCharacters(in: .whitespaces).lowercased()
        guard lower.hasPrefix("rgb"),
              let open = lower.firstIndex(of: "("), let close = lower.firstIndex(of: ")")
        else { return nil }
        let content = String(lower[lower.index(after: open)..<close])
        let parts = content.split(whereSeparator: { $0 == "," || $0 == " " }).map(String.init)
        guard parts.count >= 3 else { return nil }
        let r = (Double(parts[0]) ?? 0) / 255.0
        let g = (Double(parts[1]) ?? 0) / 255.0
        let b = (Double(parts[2]) ?? 0) / 255.0
        let a = parts.count > 3 ? (Double(parts[3]) ?? 1.0) : 1.0
        return NSColor(calibratedRed: r, green: g, blue: b, alpha: a)
    }

    /// CSS/X11 named colors → 0xRRGGBB (NSColor+String.m sColorTable).
    private static let names: [String: Int] = [
        ...
        "lightseagreen": 0x20b2aa, "lightskyblue": 0x87cefa, "lightslateblue": 0x8470ff,
        ...
        "turquoise": 0x40e0d0, "violet": 0xee82ee, "violetred": 0xd02090,
